...
Assign an outcome to an accountable agent. By default the team finishes on concrete evidence (commits, documents, observations) and you review the result; choose **Reviewed** on the assignment or its steward when every artifact should get mandatory cross-family model review. Draft PR delivery always gets one independent review of the exact commit first. Select a document passage to discuss it; the revision selector preserves access to earlier drafts. Human steering and decisions are shared within the organization. External PR publication requires a separate explicit decision.
...
- Stewards: permanent agents that own a domain, remember facts and a journal, run routines and raise signals on the home page; asked for in a conversation, never a form. See [stewards](docs/design/stewards.md).
- Routine completion by default, reviewed completion as an opt-in per assignment or steward. Workers can schedule independent review before finishing; reviewers wait for completed evidence and remain accountable to the supervisor. Late collaboration reports completion without reopening work, while explicit human steering remains actionable. Assignment status lists missing reviews, including publication evidence.
