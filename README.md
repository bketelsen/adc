...
Assign an outcome to an accountable agent. By default the team finishes on concrete evidence (commits, documents, observations) and you review the result; choose **Reviewed** on the assignment or its area when every artifact should get mandatory cross-family model review. Draft PR delivery always gets one independent review of the exact commit first. Select a document passage to discuss it; the revision selector preserves access to earlier drafts. Human steering and decisions are shared within the organization. External PR publication requires a separate explicit decision.
...
- Routine completion by default, reviewed completion as an opt-in per assignment or area. Workers can schedule independent review before finishing; reviewers wait for completed evidence and remain accountable to the supervisor. Late collaboration reports completion without reopening work, while explicit human steering remains actionable. Assignment status lists missing reviews, including publication evidence.
