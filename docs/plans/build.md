...
### Decision acceptance usability — implemented, awaiting service rollout

The Repogen run exposed duplicate bookkeeping: chat acceptance did not satisfy a plan's human-evidence requirement. Decisions now support a bounded brief, explicit approve/reject/refine outcomes and an optional server-pinned acceptance link. A single authenticated approval commits both records and wakes the target; team approvals and permission bundles retain their own effects. Tests cover stale revisions, rejection/refinement, atomic failure, requester/target wakeups, legacy team compatibility, and desktop/phone interactions with live updates. Existing unstructured requests retain their source text and need an agent-authored replacement for automatic linkage. Observations of operational facts are not inferred from approval prose.

Qualification: `make verify`, desktop/phone browser fixture, and independent Claude code review passed. No restart or mutation of the active Repogen assignment was performed for this change.
