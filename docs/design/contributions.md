...
When the accountable supervisor imports and registers an admitted candidate itself, it can hand that exact commit to a direct implementation worker for finalization and independent review. Completion recognizes the handoff only when every retained supervisor commit is covered by a completed, nonsuperseded worker's clean artifact and a current final PASS independent of both supervisor and worker model families. Candidate-stage, stale or superseded reviews cannot satisfy it. Original evidence remains attached to the supervisor, and all normal pending-work, decision, code, document and completion gates still apply.
