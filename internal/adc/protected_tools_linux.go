...
func (e *Engine) protectedTools(ctx context.Context, original Run) []copilot.Tool {
	...
	tools := []copilot.Tool{
		...
		copilot.DefineTool("adc_request_access", "Bundle missing capabilities for human review: Purpose and Tools entries with Tool ID, optional exact Arguments and Operation, and optional Constraints {Pointer, Allowed}. Exact Arguments restrict the grant to that whole object; omit them to propose a variable capability bounded by Constraints. In an active execution plan, a change or broad tool also needs Context {Action, Target, Environment, Effect, Validation, Rollback} with exact Arguments and Operation, bound to the current step and evidence. Approval covers only what was requested. This yields the run and grants nothing itself.", func(p struct {
			Purpose string
			Tools   []AccessWant
