...
(async()=>{
 ...
 try {
  ...
  await area.getByText('Cancel this obligation',{exact:true}).click();
  await area.getByLabel('Reason',{exact:true}).fill('Fixture no longer needed.');
  await area.getByRole('button',{name:'Cancel obligation',exact:true}).click();
  await area.getByText('cancelled',{exact:true}).waitFor();
