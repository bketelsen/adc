...
func (e *Engine) proposeWork(r Run, input proposalInput) (proposalResult, error) {
	...
	if input.ID == "" {
		for _, existing := range list[WorkProposal](s, "proposal", p.Org) {
			if existing.Cadence == p.Cadence && strings.EqualFold(strings.Join(strings.Fields(existing.Title), " "), strings.Join(strings.Fields(p.Title), " ")) && strings.Join(strings.Fields(existing.Scope), " ") == strings.Join(strings.Fields(p.Scope), " ") {
...
func (w *Web) proposalAction(r *http.Request, page Page) error {
	...
	case "accept":
		...
		t := Assignment{Execution: execution, ID: ID(), Org: p.Org, Proposal: p.ID, Title: p.Title, Owner: f("owner"), Account: f("account"), ExtraAccount: f("extra_account"), Creator: page.User.ID, Authority: authority}
