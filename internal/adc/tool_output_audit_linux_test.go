...
func TestIndependentInlineToolOutputAudit(t *testing.T) {
	...
	files := []string{"tool_output.go", "tool_output_test.go", "provider_tools.go", "protected_tools_linux.go", "contribution_review.go", "contributions_test.go"}
	...
	for _, name := range files {
		...
		must(t, err)
		if name == "contributions_test.go" {
			b = b[strings.Index(string(b), "func TestContributionWaitRejoinsWithoutExtendingDeadline"):]
		}
		prompt += "\nFILE " + name + "\n" + string(b) + "\nEND FILE\n"
	}
